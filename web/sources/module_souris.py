...
'''
-> Medieval Fight : Module pour la classe Souris
...
class Souris() :
    ...
    def __init__(self, jeu, attributs_jeu, sauvegarde, terrain, gestionnaire_sons) :
        ...
        assert isinstance(gestionnaire_sons, module_musique_et_sons.Gestionnaire_Son), 'gestionnaire_sons doit être de ma classe Gestionnaire_Son !'
    ...
    def personnage_est_selectionne(self):
        ...
        if 0 <= position_case[0] <= 20 and 0 <= position_case[1] <= 20:
            ...
            if selection == self.attributs_jeu.acc_selection() :
                self.jeu.changer_personnage(' ') #si le joueur reclique sur le même personnage, on enlève la sélection
            else :
    ...
    def deselectionner_bouton(self):
        ...
        if self.attributs_jeu.acc_bouton_clique() != None and time.time() - self.attributs_jeu.acc_temps_appui_bouton() > 0.3:
            ...
            #Sinon si le bouton est sauvegarder, sauvegarde la partie en cours et signale au joueur par phrase dans la console du jeu (Succès ou Échec).
                ...
                self.sauvegarde.sauvegarder(self.attributs_jeu)
    ...
    def boutons_menu_options(self) :
        ...
        if 480 <= position_curseur[0] <= 775 and 580 < position_curseur[1] < 627 :
            ...
            volume_sonore = (position_curseur[0] - 490)/ 300 # la bonne valeur est de base 480 mais j'ai augmenté pour que les joueurs puissent plus facilement sélectionner un volume de 0
            if volume_sonore < 0 :
                volume_sonore = 0
    ...
    def entrees_jeu(self, evenement) :
        ...
        if evenement.type == pygame.MOUSEBUTTONDOWN :
            ...
            if evenement.button == 1 :
                ...
                if not self.attributs_jeu.acc_deplacement_en_cours() and not self.attributs_jeu.acc_attaque_en_cours() and not self.attributs_jeu.acc_annonce_coffre() :
